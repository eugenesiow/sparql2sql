...
public final class TokenizerTextMap implements Tokenizer
{
    ...
    /*package*/ TokenizerTextMap(PeekReader reader) {
        this(reader, false) ;
    }
    
    /* package */TokenizerTextMap(PeekReader reader, boolean lineMode) {
        this.reader = reader ;
...
//            case CH_RSLASH:
    ...
    private static final boolean SupportMapping = true ; // Support for sparql2sql mapping
    ...
    private String readIRI() {
        ...
        for (;;) {
            ...
            switch(ch) {
                ...
                default:
                    if ( ch <= 0x19 )
                        warning("Illegal d character in IRI (control char 0x%02X): %s", ch, stringBuilder.toString()) ;
            }
