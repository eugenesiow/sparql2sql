...
public class TestMonetDBQuery {
	public static void main(String[] args) {
		String folderPath = "/Users/eugene/Downloads/knoesis_observations_map_snow_meta_test/";
        if (args.length > 0) {
        	folderPath = args[0];
        }
		...
		String outputPath = "/Users/eugene/Downloads/knoesis_results_monetdb/";
		if (args.length > 2) {
			outputPath = args[2];
        }
		String queryName = "q1";
		...
		String h2connection = "jdbc:monetdb://localhost/";
		if (args.length > 5) {
			h2connection = args[5];
        }
	...
	//				SQLFormatter formatter = new SQLFormatter();
					...
					if(!sql.trim().equals("")) {
						...
						try {
							Connection conn = DriverManager.getConnection(h2connection+stationName, "monetdb", "monetdb");
